...
/*             "  -S  S   storage system info\n" */
            ...
            "\n\n"
            "SURL OPTIONS:\n\n"
            "  -D      Do a recursive Get (default: not specified\n"
...
void fill_storage_type(char *option, struct soap *soap, struct ns1__srmPrepareToGetRequest *file)
{
...
void fill_lifetime(char *option, struct soap *soap, struct ns1__srmPrepareToGetRequest *file)
{
    char *err;
    long long lifetime=0;
    int size=sizeof(int);
    if(NULL == (file->desiredPinLifeTime = soap_malloc(soap,size)))
        exit(error_usage(ENOMEM,"Unable to allocate memory"));
    memset(file->desiredPinLifeTime,0,size);
    ...
    *file->desiredPinLifeTime=lifetime;
}
...
int parse_global_options_PtG(int argc, 
                             char **argv, 
                             struct soap *soap, 
                             struct ns1__srmPrepareToGetRequest *request)
{
//    struct ns1__srmPrepareToGetRequest *request;
...
{
    struct ns1__srmPrepareToGetRequest *request;
    struct ns1__srmPrepareToGetResponse_ response_p;
...
#endif
    size=sizeof(struct ns1__srmPrepareToGetRequest);
    ...
    /* call soap */
    if (soap_call_ns1__srmPrepareToGet (&soap, endpoint, "PrepareToGet",
                                        request, &response_p)) {
    ...
    response=response_p.srmPrepareToGetResponse;
    ...
    if (pollflag) {
        ...
        struct ns1__TReturnStatus *reqstatp = 
            response_p.srmPrepareToGetResponse->returnStatus;
        int soap_bool = 0;
        int r = 0;
        int i;
        memset (&sreq, 0, sizeof(sreq));
        sreq.requestToken = soap_strdup(&soapr,response_p.srmPrepareToGetResponse->requestToken);
...
//            sleep ((r++ == 0) ? 1 : DEFPOLLINT);
            ...
            soap_bool = 1;
            if (soap_call_ns1__srmStatusOfGetRequest (&soapr, endpoint,
