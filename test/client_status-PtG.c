...
int fill_surl_array(int argc, 
                    ...
                    int start, 
                    struct soap *soap, 
                    struct ns1__srmStatusOfGetRequestRequest *request)
{
    int i;
    int num=0;
    int size;
...
main(int argc, char **argv)
{
    ...
    if (soap_call_ns1__srmStatusOfGetRequest (&soap, endpoint,
    ...
    printf ("  Status=[%d/%s]", reqstatp->statusCode, reconvertStatusCode(reqstatp->statusCode));
    if (reqstatp->explanation) {
        printf(" Explanation=[%s]", reqstatp->explanation);
    }
    printf("\n");
   
    if (! repfs) {
        printf ("Returned arrayOfFileStatuses is NULL\n");
        soap_end (&soap);
        exit (1);
    }
    ...
    for (i = 0; i < repfs->__sizestatusArray; i++) {        
        ...
        if(SRM_USCOREFAILURE != (repfs->statusArray[i])->status->statusCode){
            printf ("%s",(repfs->statusArray[i])->transferURL);
            if(verbose){
                puts("");
                printf("  StatusCode=[%d/%s]", (repfs->statusArray[i])->status->statusCode, reconvertStatusCode((repfs->statusArray[i])->status->statusCode));
                if ((repfs->statusArray[i])->status->explanation)
                    printf(" Explanation=[%s]", (repfs->statusArray[i])->status->explanation);
                printf("\n");
