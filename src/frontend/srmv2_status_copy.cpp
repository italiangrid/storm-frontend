...
EXTERN_C int ns1__srmStatusOfCopyRequest(struct soap *soap,
    ...
    if ((NULL != req->arrayOfSourceSURLs) && (NULL != req->arrayOfTargetSURLs)) {
        if (req->arrayOfSourceSURLs->__sizeurlArray == req->arrayOfTargetSURLs->__sizeurlArray) {
            for (int i=0; i<req->arrayOfSourceSURLs->__sizeurlArray; ++i)
                status.add_requested_surl(req->arrayOfSourceSURLs->urlArray[i], req->arrayOfTargetSURLs->urlArray[i]);
        }
    }
    
    return __process_request_status<ns1__srmStatusOfCopyRequestResponse> (soap, req->requestToken, status, &rep->srmStatusOfCopyRequestResponse);
}
