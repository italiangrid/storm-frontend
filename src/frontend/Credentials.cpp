...
Credentials::Credentials(struct soap *soap)
{
	...
	try
	{
		cred = Credentials::get_gss_cred_id(gss_context);
	}
	...
	try
	{
		...
		X509 * x509 = Credentials::gss_cred_extract_cert(gsi_cred);
		/*
		{
		...
		STACK_OF(X509) * chain = Credentials::gss_cred_extract_cert_chain(gsi_cred);
		/*{
 ...
 */
gss_cred_id_t Credentials::get_gss_cred_id(const gss_ctx_id_t gss_context)  throw (std::invalid_argument)
{
...
globus_gsi_cred_handle_t Credentials::get_gss_cred_handle(const gss_cred_id_t gss_cred) throw (CredentialException)
{
...
X509 * Credentials::gss_cred_extract_cert(const globus_gsi_cred_handle_t gsi_cred) throw (CredentialException)
{
    // function name for error macros
    static char * funcName = "gss_cred_extract_cert";
    /* Internally a gss_cred_id_t type is a pointer to a gss_cred_id_desc */
...
STACK_OF(X509) * Credentials::gss_cred_extract_cert_chain(const globus_gsi_cred_handle_t gsi_cred) throw (CredentialException)
{
    // function name for error macros
...
char * Credentials::x509_convert_to_PEM(const X509 * x509, const STACK_OF(X509) * chain) throw (CredentialException)
{
    // function name for error macros
