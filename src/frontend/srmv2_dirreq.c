...
/*
void rpcResponseHandler_MkdirNEW(const char         *serverUrl,
                              const char         *method_name,
                              const xmlrpc_value *param_array,
                              const void         *user_data,
                              const xmlrpc_env   *faultP,
                              xmlrpc_value       *result)
{
    static const char *func = "rpcResponseHandler_MkdirNEW";
    struct RPC_ResponseHandlerInput_MkdirNEW *input;
    int error;
    xmlrpc_env env;
    xmlrpc_value *globalReturnStatusP;

    input = (struct RPC_ResponseHandlerInput_MkdirNEW *) user_data;

    srmlogit(STORM_LOG_DEBUG, func, "Inside the response handler\n");

    if (faultP->fault_occurred) {
        srmlogit(STORM_LOG_ERROR, func, "ERROR: XML-RPC Fault: %s (code: %d)\n", faultP->fault_string, faultP->fault_code);
        input->explanation = faultP->fault_string;
        input->status = SRM_USCOREINTERNAL_USCOREERROR;
        input->RPCTerminated = 1;
        return;
    }
    ...
    error = decode_globalTReturnStatus(func,&env, soap, repp->returnStatus, result);
    if (error != 0) {
		input->explanation = "Error: unable to parse the BE response or empty response";
		input->status = SRM_USCOREINTERNAL_USCOREERROR;
		xmlrpc_env_clean(&env);
        input->RPCTerminated = 1;
        return;
...
/*
int ns1__srmMkdir_implNEW(struct soap* soap, char* authorizationID, char* SURL,
		char** extraInfoKeyArray, char** extraInfoValueArray, int extraInfoArraysSize,
		ns1__TStatusCode* status, const char* explanation)
{
    static const char *func = "Mkdir";
    static const char *methodName_mkdir = "synchcall.mkdir";
    char clientdn[256], **fqans;
    int i, error, nbfqans;
    struct RPC_ResponseHandlerInput_MkdirNEW MkdirResponseHandlerInput;
    xmlrpc_env env;
    xmlrpc_value *inputParam;
    ...
    error = encode_VOMSAttributes(func, &env, soap, authorizationID, inputParam);
    if (error) {
    	*explanation = "Error encoding VOMS attributes";
        *status = SRM_USCOREINTERNAL_USCOREERROR;
        xmlrpc_DECREF(inputParam);
    ...
    error = encode_string(func, &env, authorizationID, SRM_PARAM_authorizationID, inputParam);
    if (error) {
        if (error != ENCODE_ERR_MISSING_PARAM && (! DONT_FAIL_FOR_AUTHORIZATION_ID)) {
            *status = SRM_USCOREINTERNAL_USCOREERROR;
            *explanation = "Error encoding authorizationID";
            xmlrpc_DECREF(inputParam);
            xmlrpc_env_clean(&env);
            return(SOAP_OK);
        }
    ...
    error = encode_string(func, &env, SURL, SRM_PARAM_SURL, inputParam);
    if (error) {
    	*status = SRM_USCOREINVALID_USCOREREQUEST;

    	if (error == ENCODE_ERR_MISSING_PARAM)
    	{
    		*explanation = "Error: parameter SURL is required";
    	}
    	else
    	{
    		*explanation = "Error encoding SURL parameter";
    	}
        xmlrpc_DECREF(inputParam);
        xmlrpc_env_clean(&env);
        return(SOAP_OK);
    ...
    if(extraInfoArraysSize > 0)
    {
    	if(extraInfoKeyArray == NULL || extraInfoValueArray == NULL)
    	{
    		*status = SRM_USCOREINTERNAL_USCOREERROR;
    		*explanation = "Received null extrainfo parameter with non 0 array size declared";
    		xmlrpc_DECREF(inputParam);
    		xmlrpc_env_clean(&env);
    		return(SOAP_OK);
    	}
    	xmlrpc_value *infoArray;
		infoArray = xmlrpc_array_new(&env);
    	for(int i = 0 ; i < extraInfoArraysSize; ++i)
    	{
			if (encode_TExtraInfo(func, &env, infoArray, extraInfoKeyArray[i], extraInfoValueArray[i]))
			{
					*status = SRM_USCOREINTERNAL_USCOREERROR;
					*explanation = "Error encoding storageSystemInfo";
        ...
        xmlrpc_struct_set_value(&env, inputParam, SRM_PARAM_storageSystemInfo, infoArray);
        xmlrpc_DECREF(infoArray);
    }

    srmlogit(STORM_LOG_DEBUG, func, "RPC input parameter set!\n");
    ...
    xmlrpc_client_call_asynch(xmlrpc_endpoint, methodName_mkdir, (void *) &rpcResponseHandler_MkdirNEW,
                              &MkdirResponseHandlerInput, "(S)", inputParam);
    ...
    *status = MkdirResponseHandlerInput.status;
    *explanation = *(MkdirResponseHandlerInput.explanation);
	srmlogit(STORM_LOG_INFO, func, "Request done. Status: %s\n", reconvertStatusCode(*status));
