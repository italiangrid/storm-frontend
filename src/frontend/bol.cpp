...
void
bol::load(struct ns1__srmBringOnlineRequest *req)
{
    if(NULL == req)     
...
bol::response()
{
    ...
    try{
        ...
        int n=0;
        for(std::vector<bol::surl_t>::const_iterator i = _surls.begin();
            i != _surls.end();
            ++i, ++n){
            if(NULL == _response->arrayOfFileStatuses->statusArray[n])
                _response->arrayOfFileStatuses->statusArray[n] 
                    = storm::soap_calloc<ns1__TBringOnlineRequestFileStatus>(_soap);
