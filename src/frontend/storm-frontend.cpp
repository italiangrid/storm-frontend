...
int main(int argc, char** argv)
{
    ...
    srmlogit(STORM_LOG_NONE, func, "%s=%s\n", OPTL_GRIDMAFILE.c_str(), configuration->getGridmapfile().c_str());
    srmlogit(STORM_LOG_NONE, func, "%s=%s\n", OPTL_HOST_CERT.c_str(), configuration->getHostCertFile().c_str());
    srmlogit(STORM_LOG_NONE, func, "%s=%s\n", OPTL_HOST_KEY.c_str(), configuration->getHostKeyFile().c_str());
...
process_request(void *soap) {
    ...
    soap->user = (struct srm_srv_thread_info*) mysql_connection_pool->getConnection(boost::this_thread::get_id());
