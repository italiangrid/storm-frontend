...
int storm::GetPermissionRequest::buildResponse()
{
	...
	try
	{
		m_builtResponse = storm::soap_calloc<ns1__srmGetPermissionResponse>(m_soapRequest);
		if(m_builtResponse == NULL)
		{
			srmlogit(STORM_LOG_ERROR, "storm::GetPermissionRequest::buildResponse()", "Unable to allocate memory for the response\n");
			return SOAP_EOM;
		}
		m_builtResponse->returnStatus = storm::soap_calloc<ns1__TReturnStatus>(m_soapRequest);
		if(m_builtResponse->returnStatus == NULL)
		{
			srmlogit(STORM_LOG_ERROR, "storm::GetPermissionRequest::buildResponse()", "Unable to allocate memory for the return status\n");
			return SOAP_EOM;
		}
